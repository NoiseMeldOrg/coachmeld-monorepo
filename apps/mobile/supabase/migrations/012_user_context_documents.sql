...
-- Add user_context as a valid source type if not already present
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_enum 
        WHERE enumlabel = 'user_context' 
        AND enumtypid = (
            SELECT oid FROM pg_type WHERE typname = 'source_type'
        )
    ) THEN
        ALTER TYPE source_type ADD VALUE 'user_context';
    END IF;
END $$;
