-- Fix handling of null user IDs in search function
-- The function should handle cases where no user ID is provided
...
CREATE OR REPLACE FUNCTION search_coach_documents_with_access(
    ...
    p_user_id UUID DEFAULT NULL,  -- Optional parameter after required ones
...
BEGIN
    ...
    user_context_docs AS (
        -- Get user context documents (only if user ID is provided)
        SELECT 
            cd.id,
            cd.content,
            cd.metadata,
        ...
        WHERE p_user_id IS NOT NULL  -- Only include if user ID is provided
...
-- Add comment
COMMENT ON FUNCTION search_coach_documents_with_access IS 'Search coach documents with vector similarity, including user context documents and respecting access tiers. Handles null user IDs properly.';