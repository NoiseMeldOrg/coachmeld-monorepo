/**
 * Gemini Embedding Service - 1536 Dimensions
 * Uses Gemini's dimension reduction to match pgvector limitations
 */
...
class GeminiEmbeddingService {
  constructor(apiKey) {
    ...
    this.dimensions = 1536; // Matching pgvector's limit
  ...
  async generateEmbedding(text) {
    ...
    try {
      ...
      // Generate embedding with dimension reduction
      const result = await model.embedContent({
        content: { parts: [{ text }] },
        taskType: 'RETRIEVAL_DOCUMENT',
        outputDimensionality: this.dimensions // Request 1536 dimensions
      });
...
if (require.main === module) {
  async function example() {
    ...
    console.log('Testing single embedding (1536 dimensions)...');
