...
-- Content audit trail (fixed: removed inline INDEX)
...
-- Create indexes separately (moved from CREATE TABLE)
CREATE INDEX IF NOT EXISTS idx_audit_source ON content_audit_trail(source_id);
CREATE INDEX IF NOT EXISTS idx_audit_date ON content_audit_trail(action_date);
